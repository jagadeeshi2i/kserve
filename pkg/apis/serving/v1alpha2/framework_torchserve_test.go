...
func TestFrameworkTorchServe(t *testing.T) {
	...
	scenarios := map[string]struct {
		...
		"AcceptGoodRuntimeVersion": {
			spec: TorchServeSpec{
				RuntimeVersion: DefaultTorchServeRuntimeVersion,
			},
			matcher: gomega.Succeed(),
		},
		"RejectGPUResourcesExcluded": {
			spec: TorchServeSpec{
				RuntimeVersion: DefaultTorchServeRuntimeVersionGPU,
			},
			matcher: gomega.MatchError(fmt.Sprintf(InvalidTorchServeRuntimeExcludesGPU)),
		},
		"RejectGPUResourcesIncluded": {
			...
			matcher: gomega.MatchError(fmt.Sprintf(InvalidTorchServeRuntimeIncludesGPU)),
		},
	}

	config := &InferenceServicesConfig{
		Predictors: &PredictorsConfig{
			TorchServe: PredictorConfig{
				ContainerImage:      "kfserving/TorchServeserver",
